...
async function recallEmail(req, res) {
  ...
  try {
    ...
    // ONLY the sender can recall — not even the receiver
    if (email.sender_id !== userId) {
      return res.status(403).json({ error: 'Only the sender can recall this email.' });
    }

    if (email.status === 'RECALLED') {
      return res.status(400).json({ error: 'This email has already been recalled.' });
    }

    await pool.query(
      `UPDATE emails SET status = 'RECALLED', recalled_at = NOW() WHERE id = $1`,
      [emailId]
    );
...
module.exports = { 
  ...
  markRead, markUnread, deleteEmail, replyToEmail, recallEmail , downloadAttachment
};
